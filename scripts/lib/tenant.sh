...
load_tenant() {
  ...
  source "${cfg}"
  export TENANT GCP_PROJECT LINUX_USER PORT PATH_PREFIX ROLE PLATFORM HOST
  export CHAT_APP_DISPLAY_NAME GOOGLE_CHAT_ALLOWED_USERS FUNNEL_BASE_URL SA_NAME
  export CHAT_TRANSPORT="${CHAT_TRANSPORT:-pubsub}" CHAT_PUBSUB_TOPIC CHAT_PUBSUB_SUB
}
