...
ENV_FILE="$(mktemp)"
{
  echo "HERMES_CHAT_TRANSPORT: \"${HERMES_CHAT_TRANSPORT}\""
  echo "HERMES_GATEWAY_MODE: \"${HERMES_GATEWAY_MODE}\""
  echo "GOOGLE_CHAT_PROJECT_ID: \"${GOOGLE_CHAT_PROJECT_ID}\""
  echo "GOOGLE_CHAT_MAX_MESSAGES: \"1\""
  echo "GOOGLE_CHAT_MAX_BYTES: \"16777216\""
  echo "HERMES_HOME: \"/opt/data\""
  echo "GOOGLE_CHAT_HTTP_EVENTS_SERVICE_ACCOUNT_EMAIL: \"chat@system.gserviceaccount.com\""
  echo "API_SERVER_HOST: \"0.0.0.0\""
  echo "API_SERVER_PORT: \"8080\""
  [[ -n "${GATEWAY_PROXY_URL}" ]] && echo "GATEWAY_PROXY_URL: \"${GATEWAY_PROXY_URL}\""
  [[ -n "${GOOGLE_CHAT_ALLOWED_USERS}" ]] && echo "GOOGLE_CHAT_ALLOWED_USERS: \"${GOOGLE_CHAT_ALLOWED_USERS}\""
  [[ -n "${GOOGLE_CHAT_HOME_CHANNEL}" ]] && echo "GOOGLE_CHAT_HOME_CHANNEL: \"${GOOGLE_CHAT_HOME_CHANNEL}\""
  [[ -n "${GOOGLE_CHAT_BOOTSTRAP_SPACES}" ]] && echo "GOOGLE_CHAT_BOOTSTRAP_SPACES: \"${GOOGLE_CHAT_BOOTSTRAP_SPACES}\""
  if [[ "${HERMES_CHAT_TRANSPORT}" == "pubsub" ]]; then
    echo "GOOGLE_CHAT_SUBSCRIPTION_NAME: \"${GOOGLE_CHAT_SUBSCRIPTION_NAME}\""
  fi
} > "${ENV_FILE}"
